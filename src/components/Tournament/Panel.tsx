...
const Panel: React.FC<Props> = (props) => {
  ...
  const playActualMatch = (firstWin:boolean)=>{
    props.model.draw.getActualMatch().actualPlaying = false
    props.model.draw.playActualMatch(firstWin)
    if(props.model.draw.getActualMatch()){
      props.model.draw.getActualMatch().actualPlaying = true
    }
    ...
    <Container>
      <Row className="h-100">
        ...
        <Col className="col-8">
          <Row>
            <Col>
              <Button className="panel-button-current" onClick={()=>{playActualMatch(true)}}>
                {firstCompetitor? firstCompetitor.personalDetails.name + ' ' + firstCompetitor.personalDetails.surname : ""}
              </Button>
            </Col>
            <Col>
              <Button className="panel-button-current" onClick={()=>{playActualMatch(false)}}>
